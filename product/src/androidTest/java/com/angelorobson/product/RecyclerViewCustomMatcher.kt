...
class RecyclerViewMatcher(private val recyclerViewId: Int, var position: Int = -1) {
    ...
    fun atPositionOnNestedScroll(
        position: Int, nestedScrollViewId: Int,
        nestedScrollViewPosition: Int
    ) =
        atPositionOnNestedScrollOnView(
            position,
            nestedScrollViewId, nestedScrollViewPosition, -1
        )

    fun atPositionOnNestedScrollOnView(
        position: Int, nestedScrollViewId: Int,
                ...
                if (childView == null) {
                    ...
                    if (recyclerView.id == recyclerViewId) {
                        ...
                        if (desiredRecycler is RecyclerView) {
                            childView = desiredRecycler.findViewHolderForAdapterPosition(
                                nestedScrollViewPosition
                            )?.itemView
                        } else {
                            return false
                        }
